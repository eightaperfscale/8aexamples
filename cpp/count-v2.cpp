// Example source code that shows affinity changes to correct false sharing
//
...
int main (void)
{
...
#ifdef _WIN32
		auto rc = SetThreadAffinityMask(workerThreads [i].native_handle(), 1ULL << i);
		if (rc == 0)
		{
	...
	// Output results
	uint64_t total = 0;
	std::ofstream countFile("counts-v2.txt");
	for (auto i = 0; i < workers; i ++)
	{
